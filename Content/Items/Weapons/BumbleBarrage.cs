...
public class BumbleBarrage : ModItem
{
    ...
    {
        var numProjectiles = Main.rand.Next(1, 3);

        for (var i = 0; i < numProjectiles; i++)
        {
            var newVelocity = velocity.RotatedByRandom(MathHelper.ToRadians(20));
            newVelocity *= 1f - Main.rand.NextFloat(0.1f);

            var projType = Main.rand.NextBool(2) ? ProjectileID.Bee : type;
