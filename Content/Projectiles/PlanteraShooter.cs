...
{
    private readonly Texture2D ChainTexture = ModContent.Request<Texture2D>("DartGunsPlus/Content/Projectiles/PlanteraChain").Value;
    ...
    {
        DrawLine(Main.spriteBatch, ChainTexture, Projectile.Center, Owner.Center, Color.White * Projectile.Opacity);
        return true;
    }

    public static void DrawLine(SpriteBatch spriteBatch, Texture2D texture, Vector2 start, Vector2 end, Color col)
    {
        var rotation = start.DirectionTo(end).ToRotation();
        var scale = Vector2.Distance(start, end) / texture.Height;
