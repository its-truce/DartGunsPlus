...
public class RedLaser : ModProjectile
{
    ...
    {
        var unit = Projectile.velocity * -1;
        var dustPos = player.Center + Projectile.velocity * Distance;

        for (var i = 0; i < 2; ++i)
        {
            var num1 = Projectile.velocity.ToRotation() + (Main.rand.NextBool(2) ? -1.0f : 1.0f) * 1.57f;
            var num2 = (float)(Main.rand.NextDouble() * 0.8f + 1.0f);
            var dustVel = new Vector2((float)Math.Cos(num1) * num2, (float)Math.Sin(num1) * num2);
            var dust = Main.dust[Dust.NewDust(dustPos, 0, 0, DustID.Firework_Red, dustVel.X, dustVel.Y)];
            dust.noGravity = true;
            dust.scale = 1.2f;
        ...
        {
            var offset = Projectile.velocity.RotatedBy(1.57f) * ((float)Main.rand.NextDouble() - 0.5f) * Projectile.width;
            var dust = Main.dust[Dust.NewDust(dustPos + offset - Vector2.One * 4f, 8, 8, DustID.Smoke, 0.0f, 0.0f, 100, new Color(), 1.5f)];
            dust.velocity *= 0.5f;
    ...
    private void ChargeLaser(Player player)
    {
        ...
        {
            var offset = Projectile.velocity;
            offset *= MoveDistance - 20;
            var pos = player.Center + offset - new Vector2(10, 10);
            if (Charge < MaxCharge) Charge++;
            var chargeFact = (int)(Charge / 20f);
            var dustVelocity = Vector2.UnitX * 18f;
            dustVelocity = dustVelocity.RotatedBy(Projectile.rotation - 1.57f);
            var spawnPos = Projectile.Center + dustVelocity;
            for (var k = 0; k < chargeFact + 1; k++)
            {
                var spawn = spawnPos + ((float)Main.rand.NextDouble() * 6.28f).ToRotationVector2() * (12f - chargeFact * 2);
                var dust = Main.dust[Dust.NewDust(pos, 20, 20, DustID.Firework_Red, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f)];
    ...
    private void UpdatePlayer(Player player)
    {
        ...
        {
            var diff = Main.MouseWorld - player.Center;
            diff.Normalize();
    ...
    {
        var red = new Color(241, 57, 57, 0);
        var col = Color.Lerp(Color.White, red, 0.4f);
        return col;
    }
}