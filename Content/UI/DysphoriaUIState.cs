...
public class DysphoriaUIState : ModUIState
{
    private static Texture2D _textureBorder;
    private static Texture2D _textureBar;
    private static DysphoriaPlayer DysphoriaPlayer => Main.LocalPlayer.GetModPlayer<DysphoriaPlayer>();
    private static float Segment => DysphoriaPlayer.DysphoriaMax2 / 38f; // Player max flail charge divided by the flail bar texture width
    ...
    {
        _textureBorder ??= ModContent.Request<Texture2D>("DartGunsPlus/Content/UI/DysphoriaBar", AssetRequestMode.ImmediateLoad).Value;
        _textureBar ??= ModContent.Request<Texture2D>("DartGunsPlus/Content/UI/DysphoriaFill", AssetRequestMode.ImmediateLoad).Value;
    ...
    public override void Draw(SpriteBatch spriteBatch)
    {
        ...
        if (!player.dead && player.HeldItem.ModItem is TrueDysphoria)
        {
            spriteBatch.End();
            ...
            rectangle.Width = (int)(DysphoriaPlayer.DysphoriaCurrent / Segment);
