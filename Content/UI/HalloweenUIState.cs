...
public class HalloweenUIState : ModUIState
{
    private static HalloweenPlayer HalloweenPlayer => Main.LocalPlayer.GetModPlayer<HalloweenPlayer>();
    private static float Segment => HalloweenPlayer.HalloweenMax2 / 38f; // Player max flail charge divided by the flail bar texture width
    ...
    {
        _textureBorder ??= ModContent.Request<Texture2D>("DartGunsPlus/Content/UI/HalloweenBar", AssetRequestMode.ImmediateLoad).Value;
        _textureBar ??= ModContent.Request<Texture2D>("DartGunsPlus/Content/UI/HalloweenFill", AssetRequestMode.ImmediateLoad).Value;
    ...
    public override void Draw(SpriteBatch spriteBatch)
    {
        ...
        if (!player.dead && player.HeldItem.ModItem is HalloweenHex)
        {
            spriteBatch.End();
            ...
            rectangle.Width = (int)(HalloweenPlayer.HalloweenCurrent / Segment);
